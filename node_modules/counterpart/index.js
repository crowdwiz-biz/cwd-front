...
Counterpart.prototype.getAvailableLocales = function() {
    return (
        this._registry.availableLocales ||
        Object.keys(this._registry.translations)
    );
};
...
Counterpart.prototype.onLocaleChange = Counterpart.prototype.addLocaleChangeListener = function(
    callback
) {
    this.addListener("localechange", callback);
};

Counterpart.prototype.offLocaleChange = Counterpart.prototype.removeLocaleChangeListener = function(
    callback
) {
    this.removeListener("localechange", callback);
};

Counterpart.prototype.onTranslationNotFound = Counterpart.prototype.addTranslationNotFoundListener = function(
    callback
) {
    this.addListener("translationnotfound", callback);
};

Counterpart.prototype.offTranslationNotFound = Counterpart.prototype.removeTranslationNotFoundListener = function(
    callback
) {
    this.removeListener("translationnotfound", callback);
};

Counterpart.prototype.onError = Counterpart.prototype.addErrorListener = function(
    callback
) {
    this.addListener("error", callback);
};

Counterpart.prototype.offError = Counterpart.prototype.removeErrorListener = function(
    callback
) {
...
Counterpart.prototype.translate = function(key, options) {
    ...
    var fallbackLocales = [].concat(
        options.fallbackLocale || this._registry.fallbackLocales
    );
    ...
    if (
        ...
        for (var i = 0, ii = fallbackLocales.length; i < ii; i++) {
            ...
            var fallbackKeys = this._normalizeKeys(
                fallbackLocale,
                scope,
                key,
                separator
...
Counterpart.prototype._normalizeKey = function(key, separator) {
    this._registry.normalizedKeys[separator] =
        this._registry.normalizedKeys[separator] || {};

    this._registry.normalizedKeys[separator][key] =
        this._registry.normalizedKeys[separator][key] ||
        function(key) {
            if (isArray(key)) {
                var normalizedKeyArray = key.map(
                    function(k) {
                ...
                for (var i = keys.length - 1; i >= 0; i--) {
                    if (keys[i] === "") {
                        ...
                        if (
                            this._registry.keepTrailingDot === true &&
                            i == keys.length
                        ) {
...
Counterpart.prototype._interpolate = function(entry, values) {
    ...
    try {
        return sprintf(
            entry,
            extend({}, this._registry.interpolations, values)
        );
    } catch (err) {
...
Counterpart.prototype._resolve = function(
    locale,
    scope,
    object,
    subject,
    ...
    if (isSymbol(subject)) {
        result = this.translate(
            subject,
            extend({}, options, {locale: locale, scope: scope})
        );
...
Counterpart.prototype._fallback = function(
    locale,
    scope,
    object,
    subject,
