    ...
    */
    ...
    claimVestingBalance(account, cvb, forceAll = false) {
        ...
        if (cvb.policy[0] == 1) {
            ...
            let begin_timestamp = cvb.policy[1].begin_timestamp;
            if (!/Z$/.test(begin_timestamp)) {
                begin_timestamp += "Z";
            }
            let begin_timestampDate = new Date(begin_timestamp);
            if (dynGlobalObject) {
                let block_time = dynGlobalObject.get("time");
                if (!/Z$/.test(block_time)) {
                    block_time += "Z";
                }
                let currentTimeDate = new Date(block_time);
                if (currentTimeDate > begin_timestampDate) {
                    let vesting_duration_seconds =
                        cvb.policy[1].vesting_duration_seconds;
                    let current_delta =
                    ...
                    if (
                        ...
                        balance = cvb.policy[1].begin_balance;
                        total_withdrawn =
                            cvb.policy[1].begin_balance - cvb.balance.amount;
                    }
                } else {
                    availablePercent = 0;
                }
        ...
        tr.add_type_operation("vesting_balance_withdraw", {
            ...
            amount: {
                amount: Math.floor(
                    balance * availablePercent - total_withdrawn
                ),
    ...
    proposeClaimVestingBalance(account, cvb, forceAll, proposal_account) {
        let tr = new TransactionBuilder();
        ...
        let claim_op = tr.get_type_operation("vesting_balance_withdraw", {
            fee: {amount: "0", asset_id: "1.3.0"},
            owner: account,
            vesting_balance: cvb.id,
            amount: {
