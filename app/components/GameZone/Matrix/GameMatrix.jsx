...
class GameMatrix extends React.Component {
                ...
                Apis.instance()
                    ...
                    .then(matrix => {
                        if (matrix.length > 0) {
                            ...
                            let prevMatrix =
                                "1.18." +
                                (parseInt(matrix[0].id.split(".")[2]) - 1);
                            Apis.instance()
                                .db_api()
                                .exec("get_objects", [[prevMatrix]])
                                .then(prvMtrx => {
                                    this.setState({
                                        prevMatrixFinish:
                                            prvMtrx[0].finish_block_number
                                    });
                                });
                            // STATS
                            Apis.instance()
    ...
    render() {
        ...
        let idleTime =
            parseInt(this.state.activeMatrix.start_block_number) -
            parseInt(this.state.prevMatrixFinish);
        let blocksPersent;
        ...
        if (this.state.activeMatrix.start_block_number - currentBlock < 0) {
            ...
            blocksPersent =
                ((currentBlock - startBlock + idleTime) / idleTime) * 100;
        }
...
export default GameMatrix = connect(GameMatrix, {
    listenTo() {
        return [AccountStore];
    },
    getProps() {
