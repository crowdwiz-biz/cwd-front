...
const MarketUtils = {
    ...
    parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/CWD, amount should be in CWD, value should be in USD, debt is in USD
                // buy is in USD, sell is in CWD
                // quote is USD, base is CWD
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in CWD/USD, amount should be in USD, value should be in CWD, debt is in USD
                // buy is in USD, sell is in CWD
                // quote is USD, base is CWD
